...
- Field/meta data: `FieldSpec` includes `ty: syn::Type` and `vis: Visibility`. `TypeSpec` includes `vis: Visibility` and `span: Span`. `VariantSpec` includes `span: Span` and `discriminant: Option<syn::Expr>`.
...
- Earlier versions attempted to emit `{ named, .. }` using a quote repetition over an `Option`, which fails because `quote!` expects an iterator for `*`/`+` repetitions. The DSL now assembles one of four shapes explicitly: `{ named, .. }`, `{ named }`, `{ .. }`, or `{ }`.
